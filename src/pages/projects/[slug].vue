...
const { getSingleProject, updateProject } = projectsStore
const { getProfilesByIds } = useCollabs()

watch(
  () => project.value?.name,
  () => {
    pageStore.pageData.value.title = `Project: ${project.value?.name || ''}`
  }
)
          ...
          <Table>
            ...
            <TableBody>
              <TableRow v-for="task in project.tasks" :key="task.id">
                <TableCell class="p-0">
                  <RouterLink
                    class="text-left block hover:bg-muted p-4 transition-all"
